...
class lmf:
    ...
    def clean(self):
        out,err=self.runcmd("rm init 1 rst."+self.ctrl+" mixm."+self.ctrl+" wkp."+self.ctrl+" basp*"
                           +" atm."+self.ctrl+" save."+self.ctrl+" init."+self.ctrl+" hssn."+self.ctrl+" -r")
        print("rm init 1 rst."+self.ctrl+" mixm."+self.ctrl+" wkp."+self.ctrl+" basp*"
                           +" atm."+self.ctrl+" save."+self.ctrl+" init."+self.ctrl+" hssn."+self.ctrl+" -r")
    ...
    def mksyml(self,kpts=41):
        ...
        if os.path.isfile("syml."+self.ctrl)==False:
            ...
            if is_tool("mksyml.py")==False:
                print("Make sure mksyml.py is installed. check https://github.com/santoshkumarradha/Quantum-condensed-matter-projects/tree/master/plotting%20bands for more information on how to install")
            else:
    ...
    def plot_bands(self,atoms,kpts=41):
        ...
        self.update(atoms)
        self.mksyml(kpts)
        temp_cmd=self.mpi(self.p)+" lmf -vnit=1 --band~mq~fn=syml "+self.ctrl
        print("running "+temp_cmd+"......")
        out,err=self.runcmd(temp_cmd)
        print("done\n")
        import os.path
        ...
        if is_tool("plotquestaal.py")==False:
            print("Make sure plotquestaal.py is installed. check https://github.com/santoshkumarradha/Quantum-condensed-matter-projects/tree/master/plotting%20bands for more information on how to install")
        else:
